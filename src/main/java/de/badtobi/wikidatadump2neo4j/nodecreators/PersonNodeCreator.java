 ...
 */
public class PersonNodeCreator implements NodeCreator {

    private final static String label = InstanceOf.HUMAN.getIntName();

    public void createNode(WikiDataEntry entry, Session session) {
        String dateOfBirth = EntityHelper.getDate(entry, Properties.DATE_OF_BIRTH);
        String dateOfDeath = EntityHelper.getDate(entry, Properties.DATE_OF_DEATH);
        ...
        session.run("MERGE (p:" + label + " {name:" + ValueHelper.escape(name) + ", " +
                (dateOfBirth == null ? "" : "born:" + ValueHelper.escape(dateOfBirth) + ",") +
                (dateOfDeath == null ? "" : "died:" + ValueHelper.escape(dateOfDeath) + ",") +
